...
**Preemption costs you the blocks, not necessarily the tokens.** When the KV cache runs out of blocks, the scheduler evicts a running request. [`_preempt_request`](https://github.com/vllm-project/vllm/blob/6a1acac3fe2924ca221fdf048b74f9a020dabddd/vllm/v1/core/sched/scheduler.py#L1203-L1225) frees its blocks, sets `request.num_computed_tokens = 0`, and prepends it to the waiting queue. There's no swap to host memory on this path. What it re-prefills on resume depends on whether those freed blocks survived: freeing a block returns it to the pool but leaves it in the prefix cache, so a request can match its own blocks straight back — unless the same pressure that preempted it has since handed that memory to someone else. Request D in the diagram paying 8189 tokens at step N+6 is the worst case, not the only one. If your p99 has a cliff under load, the counter declared as [`vllm:num_preemptions`](https://github.com/vllm-project/vllm/blob/6a1acac3fe2924ca221fdf048b74f9a020dabddd/vllm/v1/metrics/loggers.py#L661-L664) — `vllm:num_preemptions_total` once Prometheus suffixes it — tells you the event happened; read it next to `vllm:prefix_cache_hits` to learn what it cost. The fix is usually more KV blocks or a lower `max_num_seqs`, not a bigger batch.

*Corrected 2026-07-27. This paragraph first claimed a preempted request always re-prefills its entire context from scratch. That is only true once its blocks have actually been evicted, and freeing a block does not evict it. [Part 2](/series/inside-vllm/preemption-is-not-amnesia/) traces the block pool and what preemption really costs.*
...
That reframing is what makes the rest of the system readable. "How much memory does my request use" becomes a question about the block pool. "Why did TTFT spike" becomes a question about how many steps your prompt waited for budget. "What did that preemption actually cost me" becomes a question about the block pool — which is [part 2](/series/inside-vllm/preemption-is-not-amnesia/), where the answer turns out to be one linked list and a call to `reversed()`.
